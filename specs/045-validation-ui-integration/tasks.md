...
- [X] T001 Add the `isFrozen` boolean field mapping to `is_frozen` (defaulting to false) in `apps/api/prisma/schema.prisma` and execute `npx prisma generate`
- [X] T002 [P] Add placeholder configuration entries for `SLACK_VALIDATION_WEBHOOK_URL` and `VALIDATION_ALERT_EMAILS` in `apps/api/.env.example`
...
- [X] T003 Implement `checkWarehouseItemQuarantine(itemId, warehouseId)` in `apps/api/src/modules/operations/scope-validation.service.ts` to query if an item lot is frozen
- [X] T004 [P] Integrate the `checkWarehouseItemQuarantine` guard into the transfer posting and receive transaction service in `apps/api/src/modules/operations/transfer-post.service.ts`
- [X] T005 [P] Integrate the `checkWarehouseItemQuarantine` guard into the inventory issue post transaction service in `apps/api/src/modules/operations/issue-post.service.ts`
...
- [X] T006 [P] [US1] Write unit and integration tests for consistency equations validation in `apps/api/src/modules/admin/tests/inventory-validation.spec.ts`

### Implementation for User Story 1

- [X] T007 [P] [US1] Implement the raw SQL invariant check queries (Equations 1, 2, and 3) in `apps/api/src/modules/admin/inventory-validation.service.ts`
- [X] T008 [US1] Implement automatic item-freezing update updates on `WarehouseItem` within the validation service in `apps/api/src/modules/admin/inventory-validation.service.ts`
- [X] T009 [US1] Implement Slack webhook and SMTP notification alert triggers inside the validation service in `apps/api/src/modules/admin/inventory-validation.service.ts`
- [X] T010 [US1] Create the authenticated controller endpoint `GET /api/admin/inventory/validate` in `apps/api/src/modules/admin/inventory-validation.controller.ts`
- [X] T011 [US1] Add the cron decorator schedule `@Cron('0 1 * * *')` in `apps/api/src/modules/admin/inventory-validation.cron.ts` to trigger daily at 1:00 AM
...
- [X] T012 [P] [US2] Write Playwright integration test for Confirm Receipt interaction in `tests/e2e/operations/transfer-receive.spec.ts`

### Implementation for User Story 2

- [X] T013 [P] [US2] Add the receive stock transfer PUT endpoint handler `PUT /api/operations/transfers/:id/receive` in `apps/api/src/modules/operations/transfers/transfer.controller.ts`
- [X] T014 [US2] Render the Confirm Receipt action button in `apps/web/src/features/operations/components/transfer-viewer.tsx` only when status is `IN_TRANSIT`
- [X] T015 [US2] Connect the receipt button handler to trigger the transfer receive mutation API call in `apps/web/src/features/operations/components/transfer-viewer.tsx`
...
- [X] T016 [P] [US3] Write Playwright E2E test verifying inventory issue form posting in `tests/e2e/operations/issue-submission.spec.ts`

### Implementation for User Story 3

- [X] T017 [P] [US3] Create the query mutation hook `useSubmitIssue` in `apps/web/src/features/operations/hooks/useSubmitIssue.ts`
- [X] T018 [US3] Add the Submit button next to Save Draft on the issue form layout in `apps/web/src/features/operations/components/issue-form.tsx`
- [X] T019 [US3] Wire the Submit button to trigger the issue submission hook and update the local component view state to read-only in `apps/web/src/features/operations/components/issue-form.tsx`
...
- [X] T020 [P] [US4] Write E2E Playwright verification test for form locking in `tests/e2e/procurement/form-locking.spec.ts`

### Implementation for User Story 4

- [X] T021 [US4] Add the read-only warning Lock Banner at the top of the form view in `apps/web/src/features/procurement/components/pr-form.tsx`
- [X] T022 [US4] Add the read-only warning Lock Banner at the top of the form view in `apps/web/src/features/procurement/components/po-form.tsx`
- [X] T023 [US4] Bind all inputs, item selectors, additions, and save triggers to `disabled` if the document's state is not `DRAFT` in `apps/web/src/features/procurement/components/pr-form.tsx`
- [X] T024 [US4] Bind all inputs, item selectors, additions, and save triggers to `disabled` if the document's state is not `DRAFT` in `apps/web/src/features/procurement/components/po-form.tsx`
...
- [X] T025 [P] Run `npm run build --filter=api` to verify NestJS API compilation
- [X] T026 [P] Run `npm run typecheck --filter=web` to verify Next.js frontend TypeScript compilation
- [X] T027 Run all Playwright and Jest verification suites per `quickstart.md`
- [X] T028 Update API documentation and user guide files
