...
# First, revert the file to git clean state
os.system("git checkout -- " + filepath)
...
# 2. Insert helper function (using 1 space indentation matching the file)
...
# Normalize CRLF to LF in content to do matching reliably
content_norm = content.replace('\r\n', '\n')
locale_target_norm = locale_target.replace('\r\n', '\n')
helper_code_norm = helper_code.replace('\r\n', '\n')

content_norm = content_norm.replace(locale_target_norm, helper_code_norm)

# Convert back to CRLF
final_content = '\r\n'.join(content_norm.split('\n'))
