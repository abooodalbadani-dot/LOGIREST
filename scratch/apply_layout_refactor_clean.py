...
# 2. Add ta useTranslations hook and renderAdjustmentMobileCard helper
locale_target = " const locale = useLocale();"
helper_code = """ const ta = useTranslations('operations.adjustment');
...
if locale_target in content_norm:
    content_norm = content_norm.replace(locale_target, helper_code)
    print("Helper code inserted successfully!")
else:
    # Try with another indentation if needed
    fallback_target = "  const locale = useLocale();"
    if fallback_target in content_norm:
        content_norm = content_norm.replace(fallback_target, helper_code)
        print("Helper code inserted successfully using fallback target!")
    else:
        print("ERROR: locale_target not found!")
        exit(1)

# 3. Find and replace the two detail tier occurrences
