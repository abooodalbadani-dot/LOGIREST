...
export function Sidebar({ locale, onClose }: SidebarProps) {
 ...
 { key: 'audit', href: `/ ${locale}/admin/audit-logs`, resource: 'admin', labelKey: 'audit_log', icon: History },
 ...
 <span className={cn("text-label-xxs font-semibold uppercase text-muted-foreground/40", '')}>
 {t(group.titleKey)}
 </span>
 </div>
 )}
...
function SidebarLink({ item, pathname, t, onClick, locale }: { item: NavItem, pathname: string, t: (key: string) => string, onClick?: () => void, locale: string }) {
 ...
 onClick={onClick}
 className={cn(
 "flex items-center gap-2.5 px-6 py-2.5 rounded-xl text-label-xs font-semibold uppercase transition-all duration-[140ms] ease-out relative overflow-hidden",
 isActive
