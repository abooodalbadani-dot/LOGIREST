...
export function IssueForm() {
  ...
  const allLinesAllocated = fields.length > 0 && fields.every(
    (f) => (f as any).allocatedQuantity >= (f as any).requestedQuantity
  );
  ...
  const onSubmit = (data: any) => {
    ...
    createIssue.mutate(data, {
      onSuccess: (issue) => {
        router.push(`/operations/issues/${issue.id}`);
      },
    ...
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Input id="departmentId" {...form.register("departmentId")}
                className={form.formState.errors.departmentId ? "border-neon-error" : ""} />
              {form.formState.errors.departmentId && (
                <p className="text-sm text-neon-error">{form.formState.errors.departmentId.message}</p>
              )}
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {form.formState.errors.items?.root && (
              <p className="text-sm text-neon-error">{form.formState.errors.items.root.message}</p>
            )}
            {fields.length === 0 ? (
              ...
              fields.map((field, index) => {
                const line = field as any;
                const isAllocated = line.allocatedQuantity >= line.requestedQuantity;
                return (
                  <div key={field.id} className={`grid grid-cols-12 gap-3 items-center p-4 rounded-lg border ${isAllocated ? "border-brand-primary/30 bg-brand-primary/5" : "border-surface-2 bg-surface-1"}`}>
                    <div className="col-span-4 space-y-1">
                      ...
                      <Input
                        ...
                        className={form.formState.errors.items?.[index]?.itemId ? "border-neon-error" : ""}
                      />
                    </div>
                    ...
                    <div className="col-span-2 space-y-1 text-center">
                      ...
                      <p className={`text-lg font-bold ${isAllocated ? "text-brand-primary" : "text-neon-amber"}`} dir="ltr">
                    ...
                    <div className="col-span-1 flex items-end justify-end pb-0.5">
                      <Button type="button" variant="ghost" size="icon" className="text-neon-error hover:bg-neon-error/10" onClick={() => remove(index)}>
      ...
      {activeLineIndex !== null && (
        <FEFOLotAllocator
          ...
          itemId={(fields[activeLineIndex] as any).itemId}
          requestedQty={(fields[activeLineIndex] as any).requestedQuantity || 1}
