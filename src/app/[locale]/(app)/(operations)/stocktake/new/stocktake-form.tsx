        ...
        <PageHeader 
          ...
          actions={
            <Button 
              ...
              size="sm" 
              onClick={() => router.back()}
              className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 hover:text-foreground h-10 px-4 rounded-xl"
            >
        ...
        <Card className="bg-surface-container-low border-none shadow-2xl shadow-black/5 rounded-[2.5rem] overflow-hidden">
          <CardContent className="p-10 space-y-10">
            ...
            <div className="flex items-center gap-6 pb-8 border-b border-surface-container-high/50">
              ...
              <div>
                <h3 className="text-xl font-bold text-foreground">{t('session_parameters')}</h3>
                <p className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] mt-1 italic">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-4">
                    <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 flex items-center gap-2 px-1">
                      <Warehouse className="w-3.5 h-3.5" />
                      {tc('warehouse')}
                    </FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger className="bg-surface-container-high/30 border-none h-14 px-6 text-[11px] font-bold rounded-2xl shadow-inner shadow-black/5 focus:ring-2 focus:ring-cyan-500/20 transition-all">
                      ...
                      <SelectContent className="bg-surface-container-highest border border-surface-container-high/50 shadow-2xl rounded-2xl overflow-hidden">
                        {warehouses?.map((wh) => (
                          <SelectItem key={wh.id} value={wh.id} className="text-[11px] font-bold py-3">
                            {locale === 'ar' ? wh.nameAr : wh.nameEn}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <FormMessage className="text-[9px] font-black uppercase px-1" />
                  </FormItem>
                )}
              />
                  ...
                  <FormItem className="space-y-4">
                    ...
                    <FormControl>
                      <Input 
                        ...
                        className="bg-surface-container-high/30 border-none h-14 px-6 text-[11px] font-bold rounded-2xl shadow-inner shadow-black/5 focus:ring-2 focus:ring-cyan-500/20 transition-all" 
                        {...field} 
                      />
                    </FormControl>
                    <FormMessage className="text-[9px] font-black uppercase px-1" />
                  ...
                  <FormItem className="md:col-span-2 space-y-4">
                    ...
                    <FormControl>
                      <Textarea 
                        ...
                        className="bg-surface-container-high/30 border-none min-h-[120px] p-6 text-[11px] font-bold rounded-2xl shadow-inner shadow-black/5 focus:ring-2 focus:ring-cyan-500/20 transition-all resize-none" 
                        {...field} 
                      />
                    </FormControl>
                    <FormMessage className="text-[9px] font-black uppercase px-1" />
          ...
          <div className="p-10 bg-surface-container-medium/30 border-t border-surface-container-high/50 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
              <Button 
                ...
                variant="ghost" 
                onClick={() => router.back()} 
                className="flex-1 md:flex-none h-14 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 hover:text-foreground rounded-2xl"
              >
                {tc('cancel')}
              </Button>
              <Button
                ...
                className="flex-1 md:flex-none h-14 px-12 bg-cyan-600 hover:bg-cyan-500 text-white text-[11px] font-black uppercase tracking-[0.25em] rounded-[1.5rem] transition-all shadow-[0_0_25px_rgba(6,182,212,0.3)] hover:shadow-[0_0_40px_rgba(6,182,212,0.5)] disabled:opacity-30 disabled:grayscale group"
              >
