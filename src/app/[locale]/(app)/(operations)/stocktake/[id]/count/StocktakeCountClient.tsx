      ...
      <div className="space-y-6">
        ...
        <Card className="p-10 bg-surface-container-low border-none shadow-none rounded-[2.5rem]">
           <div className="max-w-md mx-auto mb-12">
              ...
              />
              <p className="text-center text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] mt-3">
                {t('scan_mode')}
              </p>
           </div>
              ...
              <Table>
                <TableHeader className="bg-white/[0.02]">
                  <TableRow className="hover:bg-transparent border-none">
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 h-12 px-8 w-[40%]">{common('item')}</TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 h-12 w-[30%]">{common('lot')}</TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-center h-12 w-[150px]">{t('counted_qty')}</TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-end h-12 px-8 w-[100px]">{common('uom')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {session.items.map((item) => (
                    <TableRow key={item.id} className="hover:bg-white/[0.01] transition-colors border-none group">
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          <span className="text-sm font-mono font-black text-muted-foreground/60" dir="ltr">{item.lotNumber}</span>
                          <span className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-widest" dir="ltr">{item.expiryDate}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <Input
                          type="number"
                          value={localCounts[item.itemId] ?? ''}
                          onChange={(e) => {
                             const val = parseFloat(e.target.value) || 0
                             setLocalCounts(prev => ({ ...prev, [item.itemId]: val }))
                             debouncedUpdate(item.itemId, val)
                          }}
                          className="text-center font-mono font-black h-10 bg-surface-container-medium border-none focus-visible:ring-1 focus-visible:ring-primary/30 transition-all rounded-lg"
                          dir="ltr"
                        />
                      </TableCell>
                      <TableCell className="text-end px-8 font-black text-[10px] text-muted-foreground/40 uppercase tracking-widest">
                        {item.uom}
                      </TableCell>
                    </TableRow>
                  ))}
