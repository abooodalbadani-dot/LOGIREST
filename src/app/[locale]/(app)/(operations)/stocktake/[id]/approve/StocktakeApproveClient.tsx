            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 mt-1 text-muted-foreground">
                   ...
                   <span className="text-[10px] font-black opacity-20 uppercase tracking-widest leading-none">|</span>
                   <span className="text-[10px] uppercase tracking-widest font-black opacity-40">{warehouseName}</span>
                </div>
              </div>
            </div>
          }
            ...
            <Table>
              <TableHeader className="bg-white/[0.02]">
                <TableRow className="hover:bg-transparent border-none">
                  <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 h-12 px-8 w-[25%]">{common('item')}</TableHead>
                  ...
                  <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-center h-12">{t('total_value')}</TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 h-12 px-8">{t('variance_reason')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                    ...
                    <TableRow key={item.id} className="hover:bg-white/[0.01] transition-colors border-none group">
                      <TableCell className="px-8 py-5">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[10px] font-black text-muted-foreground/30 font-mono" dir="ltr">{item.barcode}</span>
                        </div>
                      </TableCell>
                      <TableCell className="text-center font-mono text-xs font-bold text-muted-foreground/60" dir="ltr">
                        {item.snapshotQty} {item.uom}
                      </TableCell>
                      <TableCell className="text-center font-mono text-xs font-black text-foreground" dir="ltr">
                        {item.countedQty} {item.uom}
                      </TableCell>
                      <TableCell className="text-center">
                         <div className={cn(
                           "inline-flex items-center px-2 py-0.5 rounded-md font-mono font-black text-[10px]",
                      ...
                      <TableCell className="text-center font-mono text-xs font-black" dir="ltr">
        ...
        <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
          <DialogContent className="bg-surface-container-high border-none p-0 overflow-hidden rounded-[2rem] max-w-lg">
            <div className="p-8 space-y-6">
              <DialogHeader>
                <DialogTitle className="text-2xl font-black tracking-tight">{t('reject_session_title')}</DialogTitle>
