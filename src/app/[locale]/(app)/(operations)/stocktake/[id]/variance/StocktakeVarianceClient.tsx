              ...
              <Table>
                <TableHeader className="bg-white/[0.02]">
                  <TableRow className="hover:bg-transparent border-none">
                    ...
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-center h-12">{t('variance_value')}</TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 h-12 w-[30%]">{t('variance_reason')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {session.items.map((item) => {
                    ...
                    return (
                      <TableRow key={item.id} className={cn("transition-colors border-none group", hasVariance ? "bg-amber-500/[0.02] hover:bg-amber-500/[0.04]" : "hover:bg-white/[0.01]")}>
                        ...
                        <TableCell className="text-center font-mono font-black text-foreground" dir="ltr">
                          {counted} {item.uom}
                        </TableCell>
                        <TableCell className="text-center">
                           <div className={cn(
                             "inline-flex items-center gap-1.5 px-3 py-1 rounded-full font-mono font-black text-[10px] tracking-tight",
                          ...
                          {hasVariance ? (
                            <div className="space-y-1.5">
                              <Textarea
                                ...
                                className={cn(
                                  "min-h-[80px] text-sm bg-surface-container-medium border-none resize-none transition-all rounded-xl focus-visible:ring-1 focus-visible:ring-primary/30",
                                  reasonError ? "bg-amber-500/10 focus-visible:ring-amber-500/50" : ""
                                )}
                              />
                              {reasonError && (
                                <p className="text-[10px] text-amber-500 font-medium animate-in fade-in slide-in-from-top-1">
                                  {t('validation.variance_reason_min')}
                                </p>
                              )}
                            </div>
                          ) : (
                            <div className="text-xs text-muted-foreground italic flex items-center gap-1.5 justify-center">
