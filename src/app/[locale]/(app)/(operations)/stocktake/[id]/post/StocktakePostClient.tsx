    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <PermissionGate action="post" resource="operations_stocktake">
        ...
        <Card className="bg-status-error/5 border-none shadow-none rounded-[2.5rem] p-10 flex flex-col md:flex-row gap-8 items-start relative overflow-hidden">
           ...
           <div className="space-y-6 relative z-10">
              <div className="space-y-2">
                <h3 className="text-xl font-black tracking-tight text-foreground">{t('post_warning_title')}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-lg">
                <div className="p-5 bg-white/[0.02] rounded-2xl space-y-1">
                   <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/30">{t('total_adjustments')}</span>
                   <p className="text-lg font-black text-foreground">{session.items.filter(i => (i.variance || 0) !== 0).length} {t('skus')}</p>
                </div>
                <div className="p-5 bg-white/[0.02] rounded-2xl space-y-1">
                   <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/30">{t('net_financial_value')}</span>
                   <p className={cn("text-lg font-black", netImpact >= 0 ? "text-status-success" : "text-status-error")} dir="ltr">
              ...
              <div className="flex flex-col gap-4 bg-white/[0.03] p-6 rounded-2xl border border-white/[0.05]">
                 ...
                 <div className="flex flex-col md:flex-row gap-3">
                    <Input 
                      ...
                      className="bg-surface-container-lowest border-none h-12 px-6 font-black tracking-widest placeholder:text-muted-foreground/20 placeholder:tracking-normal"
                    />
                    <Button 
                      onClick={handlePost}
        ...
        <div className="flex justify-center">
          <Button variant="link" onClick={() => router.push(`/stocktake/${id}`)} className="text-muted-foreground/40 hover:text-primary transition-colors text-[10px] font-black uppercase tracking-widest">
