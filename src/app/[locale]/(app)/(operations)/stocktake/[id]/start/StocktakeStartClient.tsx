          ...
          <Card className="md:col-span-2 bg-surface-container-low border-none shadow-none rounded-[2.5rem] overflow-hidden">
            <CardContent className="p-10 space-y-10">
              <div className="flex items-center gap-6 pb-8 bg-white/[0.01]">
                ...
                <div>
                  <h3 className="text-xl font-black text-foreground tracking-tight">{t('pre_start_verification')}</h3>
                  <p className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] mt-1 italic">
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm font-black text-foreground">{warehouseName}</p>
                  </div>
                  <div className="space-y-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/30 flex items-center gap-2">
                ...
                <div className="p-8 rounded-3xl bg-surface-container-medium/30 border-none space-y-4">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-cyan-500/70">{t('session_details')}</h4>
                  <p className="text-lg font-black text-foreground tracking-tight">{session.sessionName}</p>
                  {session.description && (
                    <p className="text-sm text-muted-foreground leading-relaxed font-medium">{session.description}</p>
                  )}
                </div>
              </div>
            </CardContent>
              ...
              <div className="relative z-10 flex items-center justify-center gap-4">
                {startStocktake.isPending ? (
                  ...
                  <>
                    ...
                    <span className="text-[13px] font-black uppercase tracking-[0.3em]">{common('locked')}</span>
                  </>
                ) : (
                  <>
                    <Play className="w-6 h-6 fill-current" />
                    <span className="text-[13px] font-black uppercase tracking-[0.3em]">{t('start_session')}</span>
                  </>
                )}
              </div>
              {!isAlreadyLocked && (
