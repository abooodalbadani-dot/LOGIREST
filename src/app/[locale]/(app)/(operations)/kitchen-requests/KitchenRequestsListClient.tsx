...
export function KitchenRequestsListClient({
  ...
  const columns = useMemo<ColumnDef<KitchenRequestSummary>[]>(() => [
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <Link 
            ...
            className="font-mono tracking-normal text-[13px] font-semibold text-cyan-500 hover:text-cyan-400 transition-colors"
          >
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-xs text-foreground/80">{row.original.department_id}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-xs font-medium text-foreground/60">{row.original.requested_by}</span>
      ),
    },
    {
      accessorKey: 'status',
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <PageHeader
        ...
        actions={
          <PermissionGate requiredAction="create" requiredResource="kitchen_requests">
            <Button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        />
        <MetricCard
          title={t('statuses.REJECTED')}
          value={0}
          icon={<AlertCircle className="w-4 h-4" />}
          variant="red"
        />
        <MetricCard
          title={t('statuses.FULFILLED')}
          value={0}
