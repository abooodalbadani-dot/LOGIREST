        ...
        actions={
          <div className="flex items-center gap-3">
            <Button 
              ...
              className="rounded-xl h-11 px-6 text-[10px] font-black uppercase transition-all"
            >
            ...
            <Button 
              ...
              disabled={createRequest.isPending}
              className={cn(
                "bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl h-11 px-8 text-[10px] font-black uppercase transition-all shadow-lg", 
        ...
        <div className="bg-surface-container-low p-8 rounded-[2.5rem] border border-surface-container-high/20 shadow-xl shadow-black/5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 mb-3 flex items-center gap-2">
                <Building2 className="w-3.5 h-3.5" />
                {t('department')}
              </label>
              <Select
                onValueChange={(val) => form.setValue('departmentId', val, { shouldValidate: true })}
              >
                <SelectTrigger className="bg-surface-container-high/30 border-none h-14 px-6 text-sm font-bold rounded-2xl focus:ring-2 focus:ring-cyan-500/20">
              ...
              {form.formState.errors.departmentId && (
                <p className="text-[10px] font-bold text-red-500 uppercase px-2">{t('validation.department_required')}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {form.formState.errors.warehouseId && (
                <p className="text-[10px] font-bold text-red-500 uppercase px-2">{t('validation.warehouse_required')}</p>
              )}
            </div>
            ...
            <div className="md:col-span-2 space-y-2">
              ...
              <Input 
                ...
                className="bg-surface-container-high/30 border-none h-14 px-6 text-sm font-bold rounded-2xl focus:ring-2 focus:ring-cyan-500/20"
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground/80">{t('items')}</h3>
                <p className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-widest mt-1">{t('specify_components')}</p>
              </div>
            </div>
            <Button 
              type="button" 
              variant="outline" 
              size="sm" 
              className="h-10 px-6 border-cyan-500/30 text-cyan-500 bg-cyan-500/5 hover:bg-cyan-500 hover:text-white rounded-xl text-[10px] font-black uppercase transition-all"
          ...
          <div className="space-y-4">
            {fields.length === 0 ? (
              <div className="py-16 text-center bg-surface-container-low rounded-[2rem] border-2 border-dashed border-surface-container-high/50">
                ...
                <p className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.2em]">{t('validation.items_required')}</p>
              </div>
            ) : (
              fields.map((field, index) => {
                ...
                return (
                  <div key={field.id} className="grid grid-cols-1 md:grid-cols-[2fr_1fr_2fr_auto] gap-4 items-end p-6 rounded-[2rem] bg-surface-container-low border border-surface-container-high/20 hover:border-cyan-500/30 transition-all group">
                    <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 px-1">{tCommon('item')}</label>
                      <Select
                        defaultValue={field.itemId}
                        onValueChange={(val) => form.setValue(`items.${index}.itemId`, val, { shouldValidate: true })}
                      >
                        <SelectTrigger className="bg-surface-container-high/30 border-none h-12 px-4 text-xs font-bold rounded-xl focus:ring-2 focus:ring-cyan-500/20">
                      ...
                      {form.formState.errors.items?.[index]?.itemId && (
                        <p className="text-[9px] font-bold text-red-500 uppercase px-1">{t('validation.item_required')}</p>
                      )}
                    </div>

                    <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 px-1 text-center block">{tCommon('quantity')}</label>
                      <div className="relative">
                        <Input 
                          ...
                          className="bg-surface-container-high/30 border-none h-12 px-4 text-sm font-black text-center rounded-xl focus:ring-2 focus:ring-cyan-500/20"
                        />
                        <span className="absolute end-4 top-1/2 -translate-y-1/2 text-[9px] font-black uppercase text-muted-foreground/30">
                          {selectedItem?.uom || '---'}
                        </span>
                      </div>
                      {form.formState.errors.items?.[index]?.quantity && (
                        <p className="text-[9px] font-bold text-red-500 uppercase text-center">{t('validation.qty_positive')}</p>
                      )}
                    </div>

                    <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 px-1">{tCommon('notes')}</label>
                      <Input 
                        {...form.register(`items.${index}.notes`)}
                        placeholder={t('line_notes_placeholder')}
                        className="bg-surface-container-high/30 border-none h-12 px-5 text-xs font-bold rounded-xl focus:ring-2 focus:ring-cyan-500/20"
                      />
                    </div>
