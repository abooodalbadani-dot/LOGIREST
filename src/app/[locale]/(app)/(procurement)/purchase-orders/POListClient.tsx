...
export function POListClient({ locale }: { locale: 'ar' | 'en' }) {
  ...
  const columns = useMemo<ColumnDef<POSummary, unknown>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span dir="ltr" className="font-mono text-amber-500 font-bold tracking-wider drop-shadow-[0_0_8px_rgba(245,158,11,0.4)]">
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col text-start">
          <span className="opacity-90 font-bold text-[13px] tracking-tight">{row.original.supplier_id}</span>
          <span className="text-[9px] uppercase tracking-widest text-muted-foreground/60 font-black">{t('supplier')}</span>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col text-start">
          ...
          <span className="text-[9px] uppercase tracking-tighter opacity-30 font-black">{t('expected_date')}</span>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col text-end">
          <span dir="ltr" className="text-[13px] font-mono font-black text-foreground/90">
            {new Intl.NumberFormat(locale === 'ar' ? 'ar-SA' : 'en-US', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            }).format(row.original.supplier_total_amount)} {row.original.currency_code}
          </span>
          <span className="text-[9px] uppercase tracking-widest text-muted-foreground/60 font-black">{t('total_amount')}</span>
        </div>
      ),
    },
    {
          ...
          actions={
            <PermissionGate action="create" resource="po">
              <Link href={`/${locale}/purchase-orders/new`}>
                <Button className="h-12 px-8 bg-amber-600 hover:bg-amber-500 text-white text-[10px] font-black uppercase tracking-[0.15em] rounded-sm transition-all shadow-lg shadow-amber-900/20">
        ...
        <DataTable 
          ...
          emptyState={
            <EmptyState 
              ...
              action={
                <PermissionGate action="create" resource="po">
                  <Link href={`/${locale}/purchase-orders/new`}>
                    <Button className="h-10 px-6 bg-amber-600 hover:bg-amber-500 text-white text-[10px] font-black uppercase tracking-widest rounded-sm transition-all shadow-lg">
          ...
          filters={
            <div className="flex flex-wrap items-center gap-6 w-full py-6 px-8 border-b border-surface-variant/10">
              ...
              <Button variant="ghost" className="h-11 px-6 text-muted-foreground/60 hover:text-amber-500 text-[9px] font-black uppercase tracking-widest border border-dashed border-surface-variant/10 rounded-sm hover:bg-amber-500/5 hover:border-amber-500/20 transition-all mt-6">
