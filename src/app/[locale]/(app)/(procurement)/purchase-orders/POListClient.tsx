...
import { Plus, Filter, ShoppingBag, CheckCircle2, Clock, FileText, ArrowRight } from 'lucide-react';
...
export function POListClient({ locale }: { locale: 'ar' | 'en' }) {
  ...
  const columns: ColumnDef<POSummary, unknown>[] = [
    {
      accessorKey: 'status',
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="opacity-90 font-semibold text-xs tracking-tight">{row.original.supplier_id}</span>
          <span className="text-[9px] uppercase tracking-tighter opacity-30 font-black">Vendor Partner</span>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col items-end">
          <span className="font-mono font-bold text-cyan-500 drop-shadow-[0_0_5px_rgba(6,182,212,0.3)]">
            {row.original.total?.toLocaleString()}
          </span>
          <span className="text-[9px] opacity-40 uppercase tracking-tighter font-black">{row.original.currency_id}</span>
        </div>
      ),
    },
    {
      accessorKey: 'expected_delivery_date',
      header: t('expected_delivery_date') || 'Expected',
      cell: ({ row }) =>
        row.original.expected_delivery_date ? (
          <div className="flex flex-col">
            <span dir="ltr" className="text-[11px] font-mono font-bold text-muted-foreground/80">
              {format(new Date(row.original.expected_delivery_date), 'MMM dd, yyyy')}
            </span>
            <span className="text-[9px] uppercase tracking-tighter opacity-30 font-black text-right">ETA Boundary</span>
          </div>
  ...
  const postedCount = data?.data?.filter(p => p.status === 'POSTED').length || 0;
  const pendingCount = data?.data?.filter(p => p.status === 'DRAFT' || p.status === 'APPROVED' || p.status === 'SUBMITTED').length || 0;
    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-surface-container-low border-none rounded-2xl overflow-hidden relative group transition-all hover:bg-surface-container-medium shadow-xl shadow-primary/5">
          ...
          <CardHeader className="pb-2 relative z-10">
            <CardDescription className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Order Volume</CardDescription>
            <CardTitle className="text-4xl font-display font-bold tracking-tight text-foreground">{totalPOs}</CardTitle>
          </CardHeader>
        ...
        <Card className="bg-surface-container-low border-none rounded-2xl overflow-hidden relative group transition-all hover:bg-surface-container-medium shadow-xl shadow-primary/5">
          ...
          <CardHeader className="pb-2 relative z-10">
            <CardDescription className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Fulfilled Commitments</CardDescription>
            <CardTitle className="text-4xl font-display font-bold tracking-tight text-cyan-500">{postedCount}</CardTitle>
          </CardHeader>
          <div className="absolute bottom-0 start-0 h-0.5 w-full bg-gradient-to-r from-cyan-500/50 to-transparent" />
        </Card>
        ...
        <Card className="bg-surface-container-low border-none rounded-2xl overflow-hidden relative group transition-all hover:bg-surface-container-medium shadow-xl shadow-primary/5">
          ...
          <CardHeader className="pb-2 relative z-10">
            <CardDescription className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Pipeline Processing</CardDescription>
        ...
        <Card className="bg-surface-container-highest/20 border-none rounded-2xl overflow-hidden relative group transition-all hover:bg-surface-container-highest/30 shadow-xl shadow-primary/5">
          ...
          <CardHeader className="pb-2 relative z-10">
            <CardDescription className="text-[10px] font-black uppercase tracking-[0.2em] text-primary/60">Expenditure Drift</CardDescription>
      ...
      <div className="bg-surface-container-low shadow-2xl shadow-primary/10 rounded-2xl overflow-hidden">
        <DataTable 
          ...
          collectionName="procurement_purchase_orders"
            ...
            <div className="flex flex-wrap items-end gap-6 w-full py-6 px-8 bg-surface-container-medium/30">
              ...
              <div className="flex flex-col gap-2 min-w-[300px] flex-[2]">
                ...
                <div className="relative">
                  <Input
                    placeholder="Search Purchase Order Documents..."
