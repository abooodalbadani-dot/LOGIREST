  ...
  if (!pr || pr.status !== 'DRAFT') {
    return (
      <div className="flex flex-col h-[60vh] items-center justify-center bg-surface-container-low shadow-xl rounded-2xl p-12 text-center">
        ...
        <h2 className="text-xl font-black uppercase tracking-widest text-muted-foreground">{t('edit_not_allowed')}</h2>
        <p className="text-xs font-bold text-muted-foreground/40 mt-2 uppercase tracking-widest">{t('edit_not_allowed_desc') || 'Only draft documents can be edited.'}</p>
