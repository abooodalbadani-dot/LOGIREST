...
export function CategoryFormClient({ id, createTitle, editTitle }: Props) {
  ...
  return (
    <MasterDataFormLayout title={id ? editTitle : createTitle} backHref="../categories"
      isSaving={create.isPending || update.isPending} onSubmit={onSubmit}>
      <div className="grid gap-4">
        <div className="grid gap-1.5">
          <Label htmlFor="cat-name-ar">{t('name_ar')}</Label>
          <Input id="cat-name-ar" dir="rtl" {...register('name_ar')} />
          {errors.name_ar && <p className="text-xs text-red-400">{errors.name_ar.message}</p>}
        </div>
        <div className="grid gap-1.5">
          <Label htmlFor="cat-name-en">{t('name_en')}</Label>
          <Input id="cat-name-en" dir="ltr" {...register('name_en')} />
