...
export function FXRatesClient({ currencyId }: Props) {
  ...
  const columns = [
    { accessorKey: 'to_currency_id', header: t('to_currency'), cell: (info: any) => <span dir="ltr" className="font-mono">{info.row.original.to_currency_id}</span> },
    {
      accessorKey: 'rate',
      header: t('rate'),
      cell: (info: any) => (
        <span dir="ltr" className="font-mono font-semibold tabular-nums">{info.row.original.rate.toFixed(4)}</span>
      ),
    },
    {
      accessorKey: 'effective_date',
      header: t('effective_date'),
      cell: (info: any) => <span dir="ltr">{format(new Date(info.row.original.effective_date), 'MMM dd, yyyy')}</span>,
    },
    {
      accessorKey: '__lock',
      header: '',
