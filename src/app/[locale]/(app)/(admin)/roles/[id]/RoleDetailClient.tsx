    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-outline-low pb-8">
        <div className="space-y-4">
          <Link 
            ...
            className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-cyan-500 transition-colors"
          >
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground/60 uppercase tracking-[0.2em] font-bold max-w-xl">
              {role?.description}
            </p>
          </div>
        </div>
        ...
        {!isAdmin && (
          <div className="flex items-center gap-4">
            <Button
              ...
              className="h-12 px-8 rounded-sm bg-cyan-500 text-black hover:bg-cyan-400 disabled:bg-surface-container-highest disabled:text-muted-foreground transition-all font-black uppercase tracking-widest text-[10px] gap-2 shadow-[0_0_20px_rgba(6,182,212,0.15)]"
            >
      ...
      {isAdmin && (
        <div className="p-6 bg-rose-500/5 border border-rose-500/10 rounded-sm flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xs font-black uppercase tracking-widest text-rose-500">{t('admin_role_locked')}</h3>
            <p className="text-[10px] text-rose-500/60 font-bold uppercase tracking-widest">Administrative policies are immutable for system integrity</p>
          </div>
        </div>
      )}
      ...
      {!isValid && !isAdmin && (
        <div className="p-4 bg-amber-500/5 border border-amber-500/10 rounded-sm flex items-center gap-3">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-amber-500">{t('at_least_one_view')}</span>
        </div>
      )}
          ...
          <table className="w-full text-start border-collapse">
            <thead>
              <tr className="bg-surface-container-highest/30 border-b border-outline-low">
                <th className="py-6 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-1/3 text-start">
                  {t('module')}
                </th>
                {ACTION_KEYS.map(action => (
                  <th key={action} className="py-6 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-center">
                    {t(action)}
                  </th>
                ))}
              </tr>
                    ...
                    return (
                      <td key={action} className="py-8 px-8 text-center">
                        <div className="flex justify-center">
                          <div className={`relative flex items-center justify-center w-10 h-10 rounded-sm border transition-all ${
                            isChecked 
                              ? 'bg-cyan-500/10 border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.1)]' 
                              : 'bg-surface-container-highest/10 border-outline-low'
                          } ${isDisabled ? 'opacity-30 grayscale cursor-not-allowed' : 'hover:border-cyan-500/80'}`}>
                            <Checkbox
                              checked={isChecked}
