...
export function SettingsClient({ locale }: { locale: string }) {
  ...
  const { register, handleSubmit, formState: { errors }, watch, setValue, reset } = useForm<SystemSettings>({
    resolver: zodResolver(SettingsSchema),
  });

  const watchedBaseCurrency = watch('base_currency');
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col gap-4 border-b border-outline-low pb-8">
        <Link 
          href={`/${locale}/admin`}
          className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-operational-cyan transition-colors"
        >
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            disabled={isPending}
            className="h-12 px-8 bg-operational-cyan text-white hover:bg-operational-cyan/90 transition-all font-black uppercase tracking-widest text-[10px] gap-2 rounded-sm shadow-[0_0_20px_rgba(var(--operational-cyan-rgb),0.2)]"
          >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              ...
              />
              {errors.system_name && (
                <p className="text-[10px] font-bold text-status-error uppercase tracking-wider">{errors.system_name.message}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select 
                onValueChange={(val) => setValue('base_currency', val)}
                defaultValue={watch('base_currency')}
              >
                <SelectTrigger className="bg-surface-container-low border-outline-low rounded-sm h-12 font-bold focus:ring-1 focus:ring-operational-cyan">
          ...
          {showCurrencyWarning && (
            <div className="p-6 bg-status-warning/5 border border-status-warning/20 rounded-sm flex items-center gap-6 animate-in fade-in zoom-in-95 duration-300">
              ...
              <div className="space-y-1">
                <h3 className="text-xs font-black uppercase tracking-widest text-status-warning">
                  Critical Impact Warning
                </h3>
                <p className="text-[11px] text-status-warning/80 font-bold uppercase tracking-widest leading-relaxed">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              ...
              />
              {errors.sender_name && (
                <p className="text-[10px] font-bold text-status-error uppercase tracking-wider">{errors.sender_name.message}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.reply_to_email && (
                <p className="text-[10px] font-bold text-status-error uppercase tracking-wider">{errors.reply_to_email.message}</p>
              )}
            </div>
          </div>
        </div>
