...
import { PERMISSION_MATRIX, type UserRole } from '@/types/rbac';

const ALL_ROLES: UserRole[] = ['ADMIN', 'INV_MGR', 'WH_KEEPER', 'PROC_OFFICER', 'AUDITOR'];
...
export function UserFormClient({ id, createTitle, editTitle }: Props) {
      ...
      <form onSubmit={onSubmit} className="max-w-2xl space-y-4">
        ...
        {!isAuditor && (
          <div className="flex gap-2">
            <button type="submit" className="px-4 py-2 bg-neon-cyan text-surface-0 rounded font-medium hover:bg-neon-cyan/80 transition-colors">
              {id ? (t('edit_user').includes('تعديل') ? 'حفظ' : 'Save') : (t('create_user').includes('إنشاء') ? 'إنشاء' : 'Create')}
            </button>
            <button type="button" onClick={() => router.push('../users')} className="px-4 py-2 bg-surface-3 text-on-surface rounded hover:bg-surface-2 transition-colors">
