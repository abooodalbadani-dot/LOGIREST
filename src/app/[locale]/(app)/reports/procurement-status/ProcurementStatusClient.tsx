...
import { useProcurementStatusReport } from '@/features/reports/hooks/useReports';
...
export default function ProcurementStatusClient() {
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      meta: { numeric: true },
      cell: ({ row }) => formatCurrency(row.getValue('total'), row.original.currency),
    },
    {
      accessorKey: 'status',
      ...
      cell: ({ row }) => (
        <StatusBadge 
          ...
          variant={row.getValue('status') === 'RECEIVED' ? 'success' : 'operational'}
        />
      ),
    },
  ];
