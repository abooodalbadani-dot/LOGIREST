  ...
  const columns: ColumnDef<NotificationTemplateRow>[] = [
    {
      ...
      header: t('code'),
      cell: ({ row }) => (
        <span dir="ltr" className="font-mono text-xs text-cyan-500 font-bold bg-cyan-500/5 px-2 py-0.5 rounded border border-cyan-500/10">
          {row.original.code}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm font-medium">{row.original.trigger_event}</span>
      ),
    },
    {
