         ...
         <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex flex-col gap-1 w-full md:w-auto">
               ...
               <div className="flex items-center gap-2">
                  <span className="text-[10px] font-black text-muted-foreground/60/40 uppercase tracking-[0.3em]">
                     {query 
            ...
            <Card className="p-8 rounded-3xl border-surface-variant/10 bg-surface-container-low/40">
               <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-3">
                     ...
                     <h4 className="text-[11px] font-black uppercase tracking-[0.2em]">{t('refine')}</h4>
                  </div>
               ...
               <div className="space-y-8">
                  <div className="space-y-4">
                     <span className="text-[9px] font-black text-muted-foreground/60/40 uppercase tracking-widest ps-1">{t('main_category')}</span>
                     <div className="flex flex-col gap-2">
                        {['All', 'Food', 'Packaging', 'Equipment'].map(c => (
                          <button key={c} className="flex items-center justify-between px-5 py-3 rounded-xl bg-surface-container-low border border-surface-variant/10 hover:border-operational-cyan/30 transition-all group">
                             <span className="text-[11px] font-black uppercase tracking-tight text-muted-foreground/60 group-hover:text-operational-cyan">{c}</span>
                  ...
                  <div className="space-y-4">
                     <span className="text-[9px] font-black text-muted-foreground/60/40 uppercase tracking-widest ps-1">{t('operational_status')}</span>
                     <div className="flex flex-wrap gap-2">
                        {['Active', 'Draft', 'Alert'].map(s => (
                           <Badge key={s} className="bg-surface-container-low border border-surface-variant/10 text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg cursor-pointer hover:bg-operational-cyan/10 hover:text-operational-cyan transition-all">
                              {s}
                           </Badge>
                        ))}
                     </div>
            ...
            <Card className="p-8 rounded-3xl border-surface-variant/10 bg-surface-container-low/40">
               ...
               <div className="space-y-3">
                  {['Lot #992', 'Premium Rice', 'Supplier Al-Baraka'].map(s => (
                    <div key={s} className="flex items-center justify-between text-[10px] font-black text-muted-foreground/60/40 hover:text-operational-cyan cursor-pointer transition-colors px-2 py-1">
                       <span>{s}</span>
            ...
            {isLoading ? (
                    ...
                    <div key={section.id} className="space-y-6">
                       <div className="flex items-center gap-4 px-4">
                          ...
                          <div className="flex flex-col gap-0.5">
                             <h3 className="text-xl font-black tracking-tight text-foreground uppercase">{section.title}</h3>
                             <span className="text-[9px] font-black text-muted-foreground/60/40 uppercase tracking-widest">{sectionResults.length} {t('records')}</span>
                          </div>
                          ...
                          {sectionResults.map(result => (
                            <a key={result.id} href={result.link} className="group flex flex-col p-8 rounded-3xl bg-surface-container-low/60 border border-surface-variant/10 hover:bg-surface-container-low hover:scale-[0.98] active:scale-95 transition-all duration-500 relative overflow-hidden">
                               <div className="flex items-start justify-between mb-8">
                                  ...
                                  {result.status && (
                                    <Badge className="bg-operational-cyan/10 text-operational-cyan border-none text-[9px] font-black px-3 h-6 rounded-full uppercase tracking-widest">
                                       {result.status}
                                    </Badge>
                                  )}
                               </div>
                               ...
                               <div className="space-y-1 mb-8">
                                  <h5 className="text-lg font-black tracking-tight group-hover:text-operational-cyan transition-colors uppercase">{result.title}</h5>
                                  <p className="text-[11px] font-black text-muted-foreground/60/40 uppercase tracking-tight">{result.subtitle}</p>
                               </div>
                               ...
                               {result.metadata && (
                                 <div className="grid grid-cols-2 gap-4 pt-6 border-t border-surface-variant/10">
                                    {Object.entries(result.metadata).map(([k, v]) => (
                                      <div key={k}>
                                         <p className="text-[9px] font-black text-muted-foreground/60/30 uppercase tracking-widest mb-1">{k}</p>
               ...
               <div className="flex flex-col items-center justify-center py-40 gap-8 text-center max-w-md mx-auto">
                  ...
                  <div className="space-y-2">
                     <h3 className="text-2xl font-black text-foreground">{t('no_matches')}</h3>
                     <p className="text-[11px] font-black text-muted-foreground/60/60 uppercase leading-relaxed tracking-tight">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 py-20">
                  {[
                    ...
                    <div key={i} className="p-10 rounded-3xl bg-surface-container-low/40 border border-surface-variant/10 space-y-6 text-center hover:bg-surface-container-low transition-colors group">
                       ...
                       <div className="space-y-2">
                          <h4 className="text-lg font-black tracking-tight uppercase text-foreground">{tip.title}</h4>
                          <p className="text-[11px] font-black text-muted-foreground/60/40 uppercase leading-relaxed tracking-tight">{tip.desc}</p>
                       </div>
                    </div>
                  ))}
               </div>
