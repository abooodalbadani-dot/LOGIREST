  ...
  const columns = useMemo<ColumnDef<Barcode, unknown>[]>(() => [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col gap-0.5">
            <span className="font-bold text-xs tracking-tight">{item.name_en}</span>
            <span className="text-[10px] opacity-40" dir="rtl">{item.name_ar}</span>
          </div>
        );
      }
    },
    { 
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col gap-0.5">
            <span className="font-bold text-xs tracking-tight">{uom.name_en}</span>
            <span className="text-[10px] opacity-40" dir="rtl">{uom.name_ar}</span>
          </div>
        );
      }
    },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1.5 font-mono">
            <span className="text-[10px] text-muted-foreground/40 font-bold italic">x</span>
            <span className="font-black text-amber-500/80">{row.original.default_qty}</span>
            {uom && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 opacity-60 hover:opacity-100 transition-opacity duration-500 pb-10 pt-4 border-t border-surface-variant/5">
        <div className="p-6 bg-surface-container-low/50 rounded-sm border border-white/5 flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h4 className="text-[10px] font-black uppercase tracking-[0.1em] text-cyan-500">{t('tips.multi_unit_title')}</h4>
        ...
        <div className="p-6 bg-surface-container-low/50 rounded-sm border border-white/5 flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h4 className="text-[10px] font-black uppercase tracking-[0.1em] text-amber-500">{t('tips.gtin_standard_title')}</h4>
            <p className="text-[10px] text-muted-foreground font-semibold leading-relaxed uppercase tracking-tight">
