...
export async function generatePDF(
 ...
 console.warn('Failed to render logo in PDF', error);
 }
 }
 ...
 // Footer
 const str = 'Page ' + drawData.pageNumber;
 doc.setFontSize(9);
 doc.setTextColor(150);
 doc.text(str, drawData.settings.margin.left, doc.internal.pageSize.getHeight() - 10);
 doc.text('LogiRest Enterprise Inventory System', doc.internal.pageSize.getWidth() - 60, doc.internal.pageSize.getHeight() - 10);
 },
 });

 // Re-render header on first page (since autoTable might start below it)
 renderHeader(doc);
