...
PGPASSWORD="${PGPASSWORD:-${DB_PASSWORD:-SecureDbPass2026!}}" psql -h "${PGHOST:-db}" -U "${PGUSER:-${DB_USER:-logirest}}" -d "${PGDATABASE:-${DB_NAME:-logirest}}" -c "INSERT INTO system_settings (key, value, version, \"createdAt\", \"updatedAt\") VALUES ('last_backup_at', NOW()::text, 1, NOW(), NOW()) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, version = system_settings.version + 1, \"updatedAt\" = NOW();" >> "$LOG_FILE" 2>&1 || true
