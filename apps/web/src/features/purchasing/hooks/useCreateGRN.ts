...
export function useCreateGRN(options?: { onConflict?: () => void }) {
  ...
  return useSafeMutation({
    ...
    mutationFn: (payload: CreateGRNPayload) => 
      apiClient.post('/procurement/grns', z.object({ data: GRNDetailSchema }), CreateGRNPayloadSchema.parse(payload)).then(res => res.data),
    onSuccess: (data) => {
