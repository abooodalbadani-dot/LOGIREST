...
import { Package, Hash, Ruler, ArrowRight, ImportIcon, Users, Database } from 'lucide-react';
...
export function ImportLandingClient({ locale }: ImportLandingClientProps) {
        ...
        {cards.map((card, idx) => {
          ...
          return (
            <button
              ...
              className="group relative flex flex-col items-start text-start p-8 rounded-2xl bg-card border border-border shadow-sm/50 transition-all duration-200 hover:scale-[1.02] hover:bg-surface-container-high hover:border-cyan-500/20 shadow-xl overflow-hidden animate-in slide-in-from-bottom-8 fill-mode-both min-w-0"
            >
              {/* Background Glow */}
              <div className={cn(
 ...
 {/* Info Box */}
 <div className="mt-8 p-6 rounded-2xl bg-card border border-border shadow-sm/50 flex items-center gap-6 max-w-2xl">
