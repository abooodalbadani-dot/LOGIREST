 ...
 if (isKitchenChief) {
  ...
  return (
   ...
   >
    <Select.Trigger
     className={cn(
      'flex items-center gap-2 px-3 py-1.5 rounded-xl bg-card border border-border shadow-sm hover:bg-surface-container transition-all group relative overflow-hidden text-left',
     )}
    >
    ...
    <Select.Portal>
     <Select.Positioner side="bottom" align="start" sideOffset={4}>
      <Select.Popup
       className={cn(
        'relative isolate z-50 max-h-64 min-w-[180px] overflow-y-auto rounded-lg bg-card border border-border shadow-sm text-foreground ambient-shadow border border-outline-low',
