...
export default function ProtectedRoute({
 ...
 const finalAction = requiredAction || action;
 const finalResource = requiredResource || resource;
 
 // If no specific permission or role is required, assume just authentication is enough
 const hasGenericAccess = !!user;
 const hasSpecificPermission = usePermission(finalAction as ActionType, finalResource as ResourceType);
 const hasRequiredRole = roles ? (user?.role && roles.includes(user.role)) : true;
 
 const hasPermission = (finalAction && finalResource) 
