...
export function ExpiredOverrideClient({ locale }: { locale: string }) {
 ...
 return (
  <div className="min-w-0 gap-6 flex-1 gap-8 flex-col flex pb-20 w-full">
  <PageHeader
   title={t('title')}
   subtitle={t('description')}
   icon={<ShieldAlert className="w-8 h-8 text-rose-500" />}
  />
  <div className="flex flex-col items-center justify-center py-32 bg-card border border-border shadow-sm rounded-[3rem] border-2 border-dashed border-white/5 min-w-0">
 ...
 return (
 <div className="flex flex-col gap-8 pb-20 min-w-0">
  <PageHeader
  title={t('title')}
  subtitle={t('description')}
  icon={<ShieldAlert className="w-8 h-8 text-rose-500" />}
