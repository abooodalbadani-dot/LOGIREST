    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span
            dir="ltr"
            className={`font-mono text-label-sm font-semibold ${isOutOfStock
      ...
      <div className="flex-1 w-full max-w-full min-w-0 overflow-hidden px-4 md:px-6 pb-32 mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-200">
        ...
        <div className="flex flex-wrap items-center gap-6 bg-card border border-border shadow-sm/30 p-6 rounded-3xl shadow-inner">
          <div className="flex items-center gap-3">
            ...
            <Select value="all">
              <SelectTrigger className="w-50 bg-card border border-border shadow-sm border-none rounded-xl h-10 text-label-xs font-semibold uppercase">
                      ...
                      <div className="flex flex-col min-w-0 flex-1 gap-1">
                        ...
                        <div className="flex items-center justify-between gap-2 mt-0.5">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {item.primaryBarcode && (
                              <div className="flex items-center gap-1 bg-surface-container px-1.5 py-0.5 rounded-md border border-border">
                                <Scan className="w-2.5 h-2.5 text-muted-foreground shrink-0" />
                                <span dir="ltr" className="font-mono text-[9px] font-bold text-muted-foreground">
                                  {item.primaryBarcode}
                                </span>
                              </div>
                            )}
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-border/40 mt-auto">
                      <div className="flex flex-col">
                        <span className="text-[9px] text-muted-foreground font-bold tracking-wider uppercase mb-0.5">
                          {tc('table_headers.available')}
                        </span>
                        <div className="flex items-baseline gap-1">
                          <span dir="ltr" className={`font-mono text-base leading-none font-bold ${isOutOfStock ? 'text-status-error' : isLowStock ? 'text-status-warning' : 'text-foreground'}`}>
                            {formatNumber(qty, currentLocale as 'ar' | 'en', 2)}
                          </span>
                          <span className="text-[10px] font-bold text-muted-foreground uppercase">
                            {item.uomCode || tc('uoms.kg')}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-1.5">
                        <PermissionGate action="update" resource="inventory">
                          <Button variant="ghost" size="icon" className="w-8 h-8 rounded-lg bg-foreground/5 hover:bg-foreground/10 text-foreground transition-colors shrink-0">
                            <Edit2 className="w-3.5 h-3.5" />
                          </Button>
                        </PermissionGate>
                        <PermissionGate action="delete" resource="inventory">
                          <Button variant="ghost" size="icon" className="w-8 h-8 rounded-lg bg-status-error/10 hover:bg-status-error/20 text-status-error transition-colors shrink-0">
                            <Trash2 className="w-3.5 h-3.5" />
                          </Button>
                        </PermissionGate>
                      </div>
