...
export function TemplateListClient({ locale }: { locale: string }) {
 ...
 if (isLoading) {
  return (
   <div className="min-w-0 gap-6 flex-1 flex-col flex space-y-6 w-full">
    <div className="h-14 w-80 bg-card/5 rounded-2xl animate-pulse" />
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
     {Array.from({ length: 6 }).map((_, i) => (
      <div key={i} className="bg-card border border-border shadow-sm/70 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-6 space-y-4 animate-pulse">
       <div className="flex justify-between items-center">
        <div className="h-6 w-24 bg-card/5 rounded-lg" />
        <div className="h-4 w-4 bg-card/5 rounded-full" />
       </div>
       <div className="space-y-2 pt-4">
        <div className="h-4 w-3/4 bg-card/5 rounded" />
        <div className="h-4 w-1/2 bg-card/5 rounded" />
       </div>
       <div className="h-8 w-full bg-card/5 rounded-lg pt-2 mt-4" />
      </div>
     ))}
    </div>
   </div>
 ...
 return (
  <div className="flex flex-col gap-8 animate-in fade-in duration-500 relative min-w-0">
   ...
   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div className="bg-card border border-border shadow-sm/60 backdrop-blur-lg border border-white/10 rounded-[2.5rem] p-6 relative overflow-hidden group shadow-xl">
    ...
    <div className="bg-card border border-border shadow-sm/60 backdrop-blur-lg border border-white/10 rounded-[2.5rem] p-6 relative overflow-hidden group sm:col-span-2 lg:col-span-1 shadow-xl">
     <div className="absolute inset-0 bg-gradient-to-tr from-cyan-500/[0.02] via-transparent to-transparent pointer-events-none" />
     <div className="flex items-center justify-between">
      <div>
       <span className="text-[10px] font-black text-muted-foreground/45 uppercase tracking-widest block mb-1">
    ...
    {data?.data?.map((template) => {
     ...
     return (
      <motion.div
       ...
       className="bg-card border border-border shadow-sm/60 backdrop-blur-lg border border-white/10 rounded-[2.5rem] p-6 shadow-2xl relative overflow-hidden group cursor-pointer hover:border-operational-cyan/35 transition-all duration-300 flex flex-col justify-between min-h-[360px] min-w-0"
      >
       ...
       <div>
         ...
         <div className="flex items-center gap-2 mt-1" onClick={(e) => e.stopPropagation()}>
          {!isSystem && (
           <button
            ...
            className="p-2 bg-card/5 border border-white/5 rounded-xl text-status-error/60 hover:text-status-error hover:bg-status-error/10 hover:border-status-error/20 transition-all duration-200"
        ...
        <div className="bg-card border border-border shadow-sm/40 border border-white/5 rounded-2xl p-4.5 space-y-4 mb-4 relative overflow-hidden group-hover:bg-card border border-border shadow-sm/70 group-hover:border-operational-cyan/15 transition-all duration-300 shadow-inner">
         <div className="flex items-center gap-1.5 pb-2.5 border-b border-white/[0.04] justify-between">
          <div className="flex items-center gap-1.5">
           <span className="w-1.5 h-1.5 rounded-full bg-card/10" />
           <span className="w-1.5 h-1.5 rounded-full bg-card/10" />
           <span className="w-1.5 h-1.5 rounded-full bg-card/10" />
          ...
          <div className="flex items-center gap-1 text-[8px] font-bold text-muted-foreground/40 bg-card border border-border shadow-sm px-1.5 py-0.5 rounded border border-white/5 uppercase">
   ...
   {data?.meta && data.meta.totalPages > 1 && (
    <div className="bg-card border border-border shadow-sm/30 backdrop-blur-md rounded-[2.5rem] border border-white/5 p-4 flex justify-center animate-in fade-in duration-700 shadow-inner">
