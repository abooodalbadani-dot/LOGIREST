...
import { CheckCircle2, Loader2, SendHorizonal, MoveRight, MoveLeft, ArrowLeft } from "lucide-react";
      ...
      <PermissionGate action="edit" resource="operations_stocktake">
        <div className="flex flex-col w-full min-h-[60vh] relative pb-24 md:pb-32" onKeyDown={handleKeyDown}>
          <div className="flex flex-col gap-4 md:gap-6 w-full pb-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full mb-4 mt-2 pb-4 border-b border-border/50">
              <Button
                variant="outline"
                size="sm"
                onClick={() => baseRouter.push(`/stocktake/${id}`)}
                className="flex items-center gap-2 px-4 py-2 bg-card hover:bg-muted border border-border text-foreground font-bold rounded-xl shadow-sm transition-all hover:scale-[1.02] w-fit"
              >
                <ArrowLeft className="w-4 h-4 rtl:rotate-180 text-primary" />
                <span className="text-xs sm:text-sm">{locale === 'ar' ? 'العودة لتفاصيل الجرد' : 'Back to Stocktake'}</span>
              </Button>
            ...
            <div className="flex flex-col gap-2 w-full mb-4 text-start">
            ...
            {disambiguationRows.length > 0 && (
              <div className="py-2 text-start">
                <div className="mb-3 px-3 py-2 bg-[#b48e67]/10 dark:bg-[#b48e67]/5 border border-[#b48e67]/20 rounded-xl">
                  <span className="text-[11px] uppercase font-bold text-foreground block tracking-wider">
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col gap-1 min-w-0">
                          ...
                          {row.expiryDate && (
                            <span className="text-[10px] text-muted-foreground font-semibold font-mono" dir="ltr">
