...
function ReportCard({ title, description, icon, href }: ReportCardProps) {
 ...
 <Link 
 href={href}
 className="group relative p-8 rounded-3xl border border-border-muted/20 bg-card border border-border shadow-sm/90 backdrop-blur-xl shadow-[0_32px_64px_-12px_rgba(0,0,0,0.3)] hover:bg-surface-container-high/95 hover:border-operational-cyan/30 transition-all duration-500 overflow-hidden animate-in fade-in zoom-in-95 fill-mode-both"
 >
 ...
 <div>
 <h3 className="font-semibold text-foreground text-title-sm uppercase">{title}</h3>
 <p className="text-label-sm text-muted-foreground/60 leading-relaxed max-w-[240px] font-bold mt-1 uppercase">{description}</p>
 </div>
 </div>
 ...
 {[1, 2, 3].map((i) => (
 <div key={i} className="w-8 h-8 rounded-full border-[2px] border-surface-container-low bg-surface-container-high flex items-center justify-center text-label-xs font-semibold text-muted-foreground/40 shadow-sm -ms-3 first:ms-0 group-hover:border-operational-cyan/10 transition-colors">
 {i}
 </div>
 ))}
 </div>
...
export function ReportsHubClient() {
 ...
 return (
  <div className="md:grid-cols-2 min-w-0 gap-6 flex-1 grid grid-cols-1 xl:grid-cols-3 flex-col flex w-full">
