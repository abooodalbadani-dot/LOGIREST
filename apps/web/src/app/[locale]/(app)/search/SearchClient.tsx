...
export default function SearchClient() {
 ...
 useEffect(() => {
  ...
  apiClient
   .get(`/search?q=${encodeURIComponent(query)}`, SearchResultsArraySchema, {
  ...
  <div className="flex duration-700 border-border flex-1 shadow-sm text-foreground min-h-screen min-w-0 animate-in bg-card gap-6 fade-in flex-col border space-y-10 w-full dark:bg-card-dark">
   ...
   <div className="max-w-[1400px] mx-auto space-y-8">
    <div className="flex flex-col md:flex-row items-center justify-between gap-6 min-w-0">
     ...
     <div className="flex items-center gap-3 bg-card border border-border shadow-sm px-4 py-2 rounded-sm border border-surface-variant/10">
    ...
    <form onSubmit={handleSearch} className="group relative max-w-4xl mx-auto">
     ...
     <Input
      ...
      className="h-24 ps-20 pe-40 rounded-3xl bg-card border border-border shadow-sm border border-border-surface text-title-lg font-bold placeholder:text-muted-foreground/60/30 focus-visible:ring-4 focus-visible:ring-operational-cyan/10 transition-all outline-none"
     />
   ...
   <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-8 min-w-0">
    
    {/* Filter Sidebar */}
    <div className="w-full lg:w-[320px] space-y-6">
     <Card className="p-8 rounded-3xl border-surface-variant/10 bg-card border border-border shadow-sm/40">
       ...
       <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2 min-w-0">
         {['All', 'Food', 'Packaging', 'Equipment'].map(c => (
          <button key={c} className="flex items-center justify-between px-5 py-3 rounded-xl bg-card border border-border shadow-sm border border-surface-variant/10 hover:border-operational-cyan/30 transition-all group">
       ...
       <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
         {['Active', 'Draft', 'Alert'].map(s => (
          <Badge key={s} className="bg-card border border-border shadow-sm border border-surface-variant/10 text-label-xxs font-semibold uppercase px-3 py-1.5 rounded-lg cursor-pointer hover:bg-operational-cyan/10 hover:text-operational-cyan transition-all">
           {s}
          </Badge>
         ))}
        </div>
    ...
    <div className="flex-1 space-y-12">
     {isLoading ? (
      <div className="flex flex-col items-center justify-center py-40 gap-6 min-w-0">
       ...
       return (
        <div key={section.id} className="space-y-6">
         <div className="flex items-center gap-4 px-4">
          <div className="w-10 h-10 rounded-2xl bg-card border border-border shadow-sm flex items-center justify-center border border-surface-variant/10">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {sectionResults.map(result => (
           <Link key={result.id} href={result.link} className="group flex flex-col p-8 rounded-3xl bg-card border border-border shadow-sm/60 border border-surface-variant/10 hover:bg-card border border-border shadow-sm hover:scale-[0.98] active:scale-95 transition-all duration-500 relative overflow-hidden min-w-0">
      ...
      <div className="flex flex-col items-center justify-center py-40 gap-8 text-center max-w-md mx-auto min-w-0">
       <div className="w-24 h-24 rounded-3xl bg-card border border-border shadow-sm flex items-center justify-center border border-border-surface">
       ...
       <Button variant="outline" onClick={() => setSearchTerm('')} className="h-12 px-8 rounded-sm border-surface-variant/10 bg-card border border-border shadow-sm text-label-xs font-semibold uppercase hover:scale-[0.98] active:scale-95">
        {t('reset')}
       </Button>
      </div>
     ) : (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 py-20">
       {[
        ...
        <div key={i} className="p-10 rounded-3xl bg-card border border-border shadow-sm/40 border border-surface-variant/10 space-y-6 text-center hover:bg-card border border-border shadow-sm transition-colors group">
