...
import { Plus, CheckCircle2, Package, Search, Barcode as BarcodeIcon, ScanLine, Camera, ArrowLeft } from 'lucide-react';
          ...
          children={
            <PermissionGate action="create" resource="master_data_barcodes">
              <div className="flex gap-4">
                ...
                <Link href={`/master-data/barcodes/mapping`} className="w-full  sm:w-auto">
                  <Button variant="outline" className="h-11 w-full px-6 border-white/5 bg-card border border-border shadow-sm hover:bg-surface-container-medium text-label-xs font-semibold uppercase rounded-sm">
                    <ScanLine className="w-3.5 h-3.5 me-2 text-foreground" />
                    {useTranslations('master_data.barcode_mapping')('title')}
                  </Button>
                </Link>
                <Link href={`/master-data/barcodes/new`} className="w-full sm:w-auto">
                  <Button className="h-11 w-full px-8 bg-primary hover:bg-primary/90 text-primary-foreground text-label-xs font-semibold uppercase rounded-sm transition-all shadow-sm shadow-primary/20">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 w-full mb-6">
          <div className="relative w-full sm:w-80 md:w-96 group">
            ...
            <Input
              ...
              className="w-full h-11 ps-10 pe-10 border-none bg-surface-container-high/40 focus:bg-surface-container-high transition-colors text-label-sm font-bold text-foreground shrink-0 rounded-lg"
            />
            <Button
              type="button"
              size="icon"
