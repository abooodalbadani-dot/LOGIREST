...
export function DepartmentListClient({ locale }: { locale: string }) {
  ...
  const { data: queryData, isLoading, isError, refetch } = useDepartments({ search: debouncedSearch || undefined });
  ...
  return (
    <div className="overflow-x-hidden min-w-0 gap-6 flex-1 fade-in max-w-full slide-in-from-bottom-4 duration-1000 animate-in flex-col flex w-full">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        <MetricCard
          label={tc('operational_compliance')}
          value="100%"
          icon={ShieldCheck}
          color="emerald"
          dir="ltr"
      ...
      <div className="flex-1 w-full min-h-[400px] md:min-h-0">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 w-full mb-6">
          ...
          {departments && departments.length > 0 && (
            <PermissionGate action="export" resource="master_data_departments">
        ...
        {!isLoading && departments.length > 0 && (
          <VirtualizedMobileGrid<Department>
            data={departments}
            estimateSize={120}
            maxHeight={600}
            className="mt-4"
            renderCard={(department: Department) => (
              <div
                key={department.id}
