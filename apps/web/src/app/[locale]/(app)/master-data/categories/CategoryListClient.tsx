...
export function CategoryListClient() {
  ...
  const { data, isLoading, isError, refetch } = useCategories({ search: debouncedSearch || undefined });
    ...
    <div className="w-full max-w-full overflow-x-hidden p-4 sm:p-6 md:p-8 flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-1000 min-w-0">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 w-full mb-2">
        ...
        {data?.data && data.data.length > 0 && (
          <PermissionGate action="export" resource="master_data_categories">
      ...
      <DataTable
        ...
        renderMobileCard={(item: Category) => (
          <div
            onClick={() => router.push(`/master-data/categories/${item.id}`)}
          ...
          >
            {/* Category Identity: Icon + Name + Code */}
            <div className="flex items-center gap-3 min-w-0 w-full">
              <div className="w-10 h-10 rounded-xl bg-operational-cyan/10 border border-operational-cyan/20 flex items-center justify-center shrink-0">
                <Layers className="w-5 h-5 text-operational-cyan" />
              </div>
              <div className="flex flex-col min-w-0 flex-1 text-start items-start">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="font-mono font-bold text-[11px] bg-surface-container-highest/60 border border-surface-variant/10 px-2.5 py-0.5 rounded text-muted-foreground text-start rtl:text-right ltr:text-left" dir="ltr">
                    {item.code || item.id}
                  </span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-border/40 w-full">
              <PermissionGate action="view" resource="master_data_categories">
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    router.push(`/master-data/categories/${item.id}`);
                  }}
                >
                  {t('view')}
                </Button>
              ...
              <PermissionGate action="edit" resource="master_data_categories">
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    router.push(`/master-data/categories/${item.id}/edit`);
                  }}
                >
                  {t('edit')}
                </Button>
