    ...
    try {
      const snapshot = await this.prisma.$transaction(
        async (tx) => {
          ...
          const ledgerTotals = await tx.$queryRaw<
          ...
          const activeAllocations = await tx.lotAllocation.findMany({
            where: {
              transferLine: {
                transfer: {
                  status: 'IN_TRANSIT',
          ...
          >`
            ...
            GROUP BY "warehouseId", "itemId", "lotId"
          `;

          const lotLedgerMap = new Map<string, Prisma.Decimal>();
          for (const total of lotLedgerTotals) {
            const key = `${total.warehouseId}_${total.itemId}_${total.lotId}`;
          ...
          const orphanedGrns = await tx.$queryRaw<
          ...
          const orphanedLots = await tx.$queryRaw<
            Array<{
              lotId: string;
              warehouseId: string;
              itemId: string;
